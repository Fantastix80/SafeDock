...
export default function SettingsView({ config, registries, onSaveGlobalSettings, onAddRegistry, onDeleteRegistry, simulatedUsers, setSimulatedUsers, activeUserProfile, setActiveUserProfile }) {
  ...
  const inputClass = "w-full px-4 py-2 text-sm rounded-xl bg-[#0A0C10] border border-white/[0.08] text-white placeholder-[#94A3B8]/30 focus:outline-none focus:border-[#F7931A]/40 transition-colors font-mono";
  const selectClass = cn(inputClass, "cursor-pointer");
  const thCl = "px-4 py-3 text-left font-mono text-xs font-medium text-[#94A3B8]/60 uppercase tracking-widest";
    ...
    <div className="grid gap-4" style={{ gridTemplateColumns: '210px 1fr' }}>
          ...
          <div key={g}>
            ...
            {TABS.filter(t => t.group === g).map(({ id, label, icon: Icon }) => (
              <button
                ...
                onClick={() => setTab(id)}
                className={cn(
                  'w-full flex items-center gap-2 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors mb-0.5',
                  tab === id
      ...
      <div className="card p-6 space-y-5">
        ...
        {tab === 'registries' && (
          <>
            ...
            <div className="space-y-1.5 mb-3">
              {registries.length === 0
                ? <p className="font-mono text-sm text-[#94A3B8]/40 py-4 text-center">Aucun registre configuré.</p>
                : registries.map(r => (
                  <div key={r.id} className="flex items-center justify-between px-4 py-3 rounded-xl bg-[#0A0C10] border border-white/[0.05] hover:border-[#F7931A]/10 transition-all">
                    <span className="text-sm">
                      <span className="text-[#F7931A] font-mono">{r.server_address}</span>
                      {' '}<span className="text-[#94A3B8]">({r.username})</span>
                    </span>
                    <button type="button" onClick={() => onDeleteRegistry(r.id)} className="text-red-400 hover:text-red-300 text-sm font-mono transition-colors">
                      Supprimer
                    </button>
                  </div>
                ))
            ...
            <form onSubmit={handleRegSubmit} className="space-y-2">
              <p className="font-mono text-sm font-medium text-[#94A3B8]/40 uppercase tracking-widest">Associer un registre</p>
        ...
        {tab === 'users' && (
          <>
              ...
              <tbody>
                {users.map(u => (
                  <tr key={u.username} className="border-b border-white/[0.03] hover:bg-white/[0.02] transition-colors">
                    <td className="px-4 py-3 font-heading font-semibold text-white">{u.username}</td>
                    <td className="px-4 py-3 font-mono text-[#94A3B8]">{u.email}</td>
            ...
            <form onSubmit={handleAddUser} className="space-y-2">
              <p className="font-mono text-sm font-medium text-[#94A3B8]/40 uppercase tracking-widest">Inviter un utilisateur</p>
              <div className="grid grid-cols-3 gap-2">
              ...
              <tbody>
                {agents.map(a => (
                  <tr key={a.name} className="border-b border-white/[0.03] hover:bg-white/[0.02] transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <span className={cn('flex items-center gap-1.5 font-mono text-sm font-medium', a.status === 'connected' ? 'text-emerald-400' : 'text-[#94A3B8]/40')}>
                        <span className={cn(
                    ...
                    <td className="px-4 py-3 text-right font-mono text-[#94A3B8]/50 text-sm">{a.version}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            <form onSubmit={handleAddAgent} className="space-y-2">
              <p className="font-mono text-sm font-medium text-[#94A3B8]/40 uppercase tracking-widest">Enrôler un agent</p>
            ...
            <div className="space-y-2">
              {[
                ...
                <div key={item.label} className="flex items-center justify-between p-3 rounded-xl bg-[#0A0C10] border border-white/[0.05] hover:border-[#F7931A]/10 transition-all">
                  ...
                  <span className="px-2 py-0.5 rounded-md text-sm font-bold bg-red-500/10 text-red-400 border border-red-500/20">Désactivé</span>
                </div>
              ))}
            </div>
            <div className="mt-4">
              <p className="font-heading text-sm font-semibold text-white mb-2">Matrice RBAC</p>
        ...
        {status && tab !== 'permissions' && (
          <p className="text-sm text-emerald-400 font-mono font-medium pt-2 border-t border-white/[0.06]">{status}</p>
        )}
      </div>
    </div>
  );
...
function FieldLabel({ children }) {
  return <label className="font-mono text-sm font-medium text-[#94A3B8]/60 uppercase tracking-wider">{children}</label>;
}

function SHead({ children }) {
  return <h3 className="font-heading text-base font-semibold text-white mb-1 pb-3 border-b border-white/[0.06]">{children}</h3>;
}

function Toggle({ label, checked, onChange }) {
  return (
    <div className="flex items-center justify-between p-4 rounded-xl bg-[#0A0C10] border border-white/[0.06] hover:border-[#F7931A]/15 transition-all">
