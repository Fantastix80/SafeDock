  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-6" style={{ gridTemplateColumns: '1fr 2fr 1fr' }}>
        {/* Score Gauge */}
        <div className="card p-7 flex flex-col items-center justify-center gap-4 text-center hover:border-[#F7931A]/30 hover:shadow-[0_0_30px_-10px_rgba(247,147,26,0.15)]">
          <p className="font-mono text-sm font-medium text-[#94A3B8] uppercase tracking-widest">Score Global</p>
          <div className="relative w-44 h-44">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <span className={cn('font-heading text-5xl font-extrabold', gradeColor(stats.globalScore))}>
                {stats.globalGrade}
              </span>
              <span className="text-sm text-[#94A3B8] mt-1 font-mono">{stats.globalScore}/100</span>
            </div>
          </div>
          <div>
            <p className={cn('font-heading text-lg font-bold', gradeColor(stats.globalScore))}>
              {gradeLabel(stats.globalGrade)}
            </p>
            <p className="text-base text-[#94A3B8] mt-1">Posture globale du parc</p>
          </div>
        </div>
        ...
        <div className="card p-7">
          <div className="mb-5">
            <p className="font-heading text-lg font-semibold text-white">Gravité des Failles (CVE)</p>
            <p className="text-base text-[#94A3B8] mt-1">Vulnérabilités cumulées détectées sur vos conteneurs</p>
          </div>
        ...
        <div className="card p-7 flex flex-col justify-between">
          <div>
            <div className="flex items-center gap-2 mb-5 pb-3 border-b border-white/[0.06]">
              ...
              <p className="font-heading text-lg font-semibold text-white">Actions recommandées</p>
            </div>
          ...
          <button
            ...
            className="mt-5 w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-base font-semibold text-[#F7931A] bg-[#F7931A]/10 hover:bg-[#F7931A]/20 border border-[#F7931A]/20 hover:border-[#F7931A]/40 transition-all duration-200"
          >
      ...
      <section>
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-2 font-heading text-base font-semibold text-white">
            <ListChecks className="w-5 h-5 text-[#94A3B8]" />
          ...
          <div className="flex gap-2">
            {[
              ...
              <button
                ...
                className={cn(
                  'px-4 py-1.5 text-sm rounded-full font-mono font-medium transition-all duration-200',
                  activeFilter === f.id
      ...
      <section>
        <div className="flex items-center justify-between mb-5">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2 text-sm text-[#94A3B8] hover:text-white hover:bg-white/[0.04] rounded-lg transition-colors font-mono"
          >
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/[0.06]">
                  {['Date', 'Conteneur', 'Image / digest', 'Statut', 'Détails', 'CVEs (C/H/M)'].map(h => (
                    <th key={h} className="px-5 py-4 text-left font-mono text-xs font-medium text-[#94A3B8]/60 uppercase tracking-widest">
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {auditLogs.length === 0 ? (
                  <tr>
                    <td colSpan="6" className="px-5 py-12 text-center text-[#94A3B8]/40 font-mono text-sm">
                  ...
                  auditLogs.map(log => (
                    <tr key={log.id} className="border-b border-white/[0.03] hover:bg-white/[0.02] transition-colors">
                      <td className="px-5 py-4 text-[#94A3B8] font-mono whitespace-nowrap">{formatDate(log.timestamp)}</td>
                      <td className="px-5 py-4 font-medium text-white">{log.container_name}</td>
                      <td className="px-5 py-4 font-mono text-[#94A3B8]/70">{log.image_ref || '-'}</td>
                      <td className={cn('px-5 py-4 font-mono font-bold text-sm tracking-wider', logStatusStyle(log.status))}>
                        {logStatusLabel(log.status)}
                      </td>
                      <td className="px-5 py-4 text-[#94A3B8] max-w-[220px] truncate">{log.message}</td>
                      <td className="px-5 py-4 text-center font-mono text-[#94A3B8]">
...
function KpiCard({ icon, iconClass, value, label, glow }) {
  return (
    <div className={cn('card px-8 py-7 flex items-center gap-6 hover:border-white/[0.15] transition-all duration-300', glow)}>
      <div className={cn('w-16 h-16 rounded-2xl flex items-center justify-center shrink-0', iconClass)}>
        {icon}
      </div>
      <div>
        <p className="font-heading text-5xl font-bold text-white leading-none">{value}</p>
        <p className="text-base text-[#94A3B8] mt-2.5 font-mono">{label}</p>
      </div>
    </div>
  );
}
...
function ActionItem({ badge, badgeClass, name, desc }) {
  return (
    <div className="flex gap-3 text-base leading-relaxed">
      <span className={cn('shrink-0 px-2 py-0.5 rounded-md font-mono text-xs font-bold h-fit tracking-wide', badgeClass)}>
        {badge}
      </span>
      <div>
...
function CveChart({ counts }) {
  ...
  const chartH = 240, barW = 64, gap = 56, paddingLeft = 32, paddingBottom = 28, paddingTop = 20;
  ...
  return (
    <svg viewBox={`0 0 ${chartW} ${chartH}`} width="100%" height="100%" style={{ maxHeight: '240px' }}>
      {[1, 2, 3].map(i => {
...
function ContainerCard({ c, onClick }) {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="card p-6 text-left hover:-translate-y-0.5 hover:border-[#F7931A]/30 hover:shadow-[0_0_25px_-8px_rgba(247,147,26,0.2)] transition-all duration-300 group cursor-pointer"
    >
      <div className="flex items-start justify-between gap-3 mb-4">
        <div className="min-w-0">
          <p className="font-heading text-lg font-semibold text-white truncate">{c.name}</p>
          <p className="text-sm text-[#94A3B8] mt-1 font-mono truncate">{c.image_name}:{c.image_tag}</p>
          <p className="text-sm text-[#94A3B8]/50 mt-0.5 font-mono">{c.host_name}</p>
        </div>
        <div className={cn('w-12 h-12 rounded-xl flex items-center justify-center font-heading text-lg font-extrabold shrink-0', scoreCol, scoreBg)}>
          {c.grade}
        </div>
      </div>
      ...
      <div className="flex items-center justify-between font-mono text-sm text-[#94A3B8]">
        <span>Score : <span className={cn('font-bold', scoreCol)}>{c.score}/100</span></span>
        {c.update_available ? (
          <span className="flex items-center gap-1.5 text-amber-400">
            <CloudDownload className="w-4 h-4" /> Màj dispo
          </span>
        ) : (
...
function Pill({ pass, children }) {
  return (
    <span className={cn(
      'inline-flex items-center gap-1.5 font-mono text-sm px-2.5 py-1 rounded-md font-medium',
