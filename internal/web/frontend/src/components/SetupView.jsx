...
import { ShieldCheck, Lock, Loader2, Smartphone, KeyRound, Copy, Check, User as UserIcon, KeySquare, CheckCircle2 } from 'lucide-react';
import QRCode from 'qrcode';
import PasswordChecklist from './PasswordChecklist';
import { checkPassword } from '../lib/passwordPolicy';

// Assistant de configuration initiale (first-run). S'affiche tant qu'aucun compte
// administrateur n'existe. L'opérateur saisit le jeton journalisé au démarrage,
// crée son compte (e-mail = identifiant + mot de passe), puis enrôle son MFA.
// phases : token | identity | enroll | backup | done
export default function SetupView({ onComplete }) {
  const [phase, setPhase] = useState('token');
  ...
  const submitIdentity = async (e) => {
    ...
    try {
      const res = await fetch('/api/setup/complete', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ token: token.trim(), email: email.trim(), first_name: firstName, last_name: lastName, password }),
      });
      const d = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(d.error || `Erreur serveur (HTTP ${res.status})`);
      // Compte créé + cookie de pré-auth posé → on enchaîne sur l'enrôlement MFA
      // via les endpoints existants.
      const mfa = await fetch('/api/login/setup-mfa', {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: '{}',
      });
      const md = await mfa.json().catch(() => ({}));
      if (!mfa.ok) throw new Error(md.error || `Erreur serveur (HTTP ${mfa.status})`);
      setSecret(md.secret || ''); setQrSource(md.otpauth_uri || ''); setCode('');
      setPhase('enroll');
    } catch (err) { setError(err.message); } finally { setBusy(false); }
  };
  ...
  const finish = () => { if (onComplete) onComplete(); else window.location.assign('/'); };
    ...
    <div className="flex items-center justify-center min-h-screen bg-[#030304] px-4 py-8">
      ...
      <div className="relative w-full max-w-sm">
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-sm text-[#94A3B8] font-mono mt-1">Configuration initiale</p>
        </div>

        <div className="bg-[#0F1115] border border-white/[0.08] rounded-2xl p-6 shadow-xl">
          {/* Étape 1 : jeton de configuration */}
          {phase === 'token' && (
            <form onSubmit={submitToken}>
              <Header icon={KeySquare} title="Bienvenue sur SafeDock" />
              <p className="text-sm text-[#94A3B8] leading-relaxed mb-4">
                Pour sécuriser la première configuration, collez le <strong className="text-white">jeton à usage unique</strong>
                {' '}affiché dans les journaux du serveur (<code>docker logs safedock-app</code>).
              </p>
              <Label>Jeton de configuration</Label>
              <InputWithIcon icon={KeySquare} value={token} onChange={setToken} autoFocus placeholder="collez le jeton ici" mono />
              {error && <ErrorMsg>{error}</ErrorMsg>}
              <SubmitBtn busy={busy} disabled={!token.trim()}>Continuer</SubmitBtn>
            </form>
          )}

          {/* Étape 2 : identité + mot de passe */}
          {phase === 'identity' && (
            <form onSubmit={submitIdentity}>
              <Header icon={ShieldCheck} title="Votre compte administrateur" />
              <p className="text-sm text-[#94A3B8] leading-relaxed mb-4">
                Votre <strong className="text-white">adresse e-mail</strong> servira d'identifiant de connexion
                {' '}et d'adresse pour vos alertes.
              </p>
              <Label>Adresse e-mail (identifiant)</Label>
              <InputWithIcon icon={UserIcon} value={email} onChange={setEmail} autoFocus placeholder="vous@entreprise.com" />
              <div className="grid grid-cols-2 gap-3 mt-3">
                <div>
                  <Label>Prénom</Label>
                  <InputWithIcon icon={UserIcon} value={firstName} onChange={setFirstName} placeholder="Prénom" />
                </div>
                <div>
                  <Label>Nom</Label>
                  <InputWithIcon icon={UserIcon} value={lastName} onChange={setLastName} placeholder="Nom" />
                </div>
              </div>
              ...
              {error && <ErrorMsg>{error}</ErrorMsg>}
              <SubmitBtn busy={busy} disabled={!email.trim() || !checkPassword(password).ok || !confirm}>Créer le compte</SubmitBtn>
            </form>
          )}

          {/* Étape 3 : enrôlement MFA */}
          {phase === 'enroll' && (
            <form onSubmit={submitCode}>
              ...
              {error && <ErrorMsg>{error}</ErrorMsg>}
              <SubmitBtn busy={busy} disabled={code.length !== 6}>Activer et terminer</SubmitBtn>
            </form>
          )}
          ...
          {phase === 'backup' && (
            <div>
              ...
              <SubmitBtn busy={false} disabled={false} onClick={finish}>J'ai sauvegardé mes codes — Accéder à SafeDock</SubmitBtn>
            </div>
          )}
          ...
          {phase === 'done' && (
            <div className="text-center py-4">
              ...
              <h2 className="font-heading text-base font-semibold text-white mb-2">SafeDock est configuré</h2>
              <p className="text-sm text-[#94A3B8] leading-relaxed mb-5">Votre compte administrateur est prêt.</p>
              <button type="button" onClick={finish}
                className="inline-flex items-center justify-center gap-2 rounded-xl px-4 py-2.5 font-mono font-semibold text-sm bg-gradient-to-r from-[#EA580C] to-[#F7931A] text-black hover:opacity-90 transition-opacity">
                Accéder à SafeDock
              </button>
            </div>
          )}
...
function SafeDockLogo({ size = 56 }) {
  return (
    <svg viewBox="0 0 40 40" width={size} height={size} fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
      <path d="M20 3L33 9V21C33 29 20 35 20 35C20 35 7 29 7 21V9Z" stroke="url(#slg1)" strokeWidth="2.5" strokeLinejoin="round" />
      <path d="M20 12L27 15L20 18L13 15Z" fill="url(#slg2)" opacity="0.9" />
      <path d="M13 15L20 18V26L13 23Z" fill="#EA580C" opacity="0.75" />
      <path d="M20 18L27 15V23L20 26Z" fill="#F7931A" />
      <defs>
        <linearGradient id="slg1" x1="7" y1="3" x2="33" y2="35" gradientUnits="userSpaceOnUse">
          <stop stopColor="#F7931A" /><stop offset="1" stopColor="#FFD600" />
        </linearGradient>
        <linearGradient id="slg2" x1="13" y1="12" x2="27" y2="18" gradientUnits="userSpaceOnUse">
