...
export default function AgentsView() {
  ...
  const thClass = "px-4 py-3 text-left font-mono text-[10px] font-medium text-[#94A3B8]/60 uppercase tracking-widest";
                ...
                <tbody>
                  {agents.map(a => (
                    <tr key={a.name} className="border-b border-white/[0.03] hover:bg-white/[0.02] transition-colors">
                      ...
                      <td className="px-4 py-3 text-right font-mono text-[#94A3B8]/50 text-[10px]">{a.version}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="card">
            ...
            {showGuide && (
              <div className="px-4 pb-4 space-y-3 border-t border-white/[0.06] pt-3">
                ...
                <div className="bg-[#0A0C10] border border-white/[0.08] rounded-xl p-3 font-mono text-[11px] text-[#F7931A] overflow-x-auto whitespace-nowrap">
                  curl -sSL https://safedock.local/install.sh | bash -s -- --token sd_agent_tok_8a92b8cd19f08831 --server 192.168.1.90
                </div>
                <div>
                  <p className="text-[11px] font-semibold text-white mb-1.5">Pré-requis :</p>
                  <ul className="text-[11px] text-[#94A3B8] space-y-1 list-disc list-inside leading-relaxed">
...
function FormField({ label, placeholder, value, onChange }) {
  return (
    <div className="space-y-1">
      <label className="font-mono text-[10px] font-medium text-[#94A3B8]/70 uppercase tracking-wider">{label}</label>
      <input
        type="text"
